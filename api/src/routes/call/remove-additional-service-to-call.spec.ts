...
describe("Remove additional service to call [PATCH] /calls/:callId/service/:serviceId", () => {
  it("should be able to remove additional service to call", async () => {
    ...
    const response = await request(app.server)
      .patch(`/calls/${call.id}/service/${serviceResponse.body.serviceId}`)
      .set("Cookie", token)
      .send()
  ...
  it("should not be able to remove additional service to inexistent call", async () => {
    ...
    const response = await request(app.server)
      .patch(`/calls/${randomUUID()}/service/${randomUUID()}`)
      .set("Cookie", token)
      .send({
    ...
    expect(response.body).toHaveProperty("message", "Call or service not found")
  })

  it("should not be able to remove additional inexistent service to call", async () => {
    const { token, user: technician } = await createAndAuthUser(app, {
      role: "TECHNICIAN",
      hours: makeTechnician().hours,
    })
    ...
    const response = await request(app.server)
      .patch(`/calls/${call.id}/service/${randomUUID()}`)
      .set("Cookie", token)
      .send()
